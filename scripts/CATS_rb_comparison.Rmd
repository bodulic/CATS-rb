...
COMM_EL_SET_REL_BREAKS <- as.numeric(unlist(strsplit(ext_args[, args][12], ",")))
...
TR_ALN_PROP_BREAKS <- as.numeric(unlist(strsplit(ext_args[, args][22], ",")))
STR_INC_ALN_PROP <- as.numeric(ext_args[, args][23])
STR_INC_MAX_OVL_PROP <- as.numeric(ext_args[, args][24])
EXON_PER_TR_N_BREAKS <- as.numeric(unlist(strsplit(ext_args[, args][25], ",")))
...
ISO_N_BREAKS <- as.numeric(unlist(strsplit(ext_args[, args][30], ",")))
...
import_data <- function(import_regex) {
 ...
 imported_dt <- list()
 if (import_regex == "mapped.tsv" | import_regex == "lengths.tsv") {
 ...
 } else if (import_regex == "general_statistics_table.tsv") {
  for (i in 1 : length(files_in_directory)) {
   if (i == 1) {
...
analyse_numerical_variable <- function(dt_to_analyse, variable_to_plot, plot_axis_title_filename) {
 plotted_value_min <- dt_to_analyse[, min(.SD[[1]]), .SDcols = variable_to_plot]
 plotted_value_max <- dt_to_analyse[, quantile(.SD[[1]], RP_MAX_QUANTILE), .SDcols = variable_to_plot]
...
analyse_categorical_variable <- function(dt_to_analyse, variable_to_plot, break_vec, plot_legend_title_filename, y_axis_label) {
 ...
 variable_bp <- ggplot(data = dt_to_analyse, aes(x = transcriptome, fill = .data[[variable_to_plot_cat]])) +
  ...
  guides(fill = guide_legend(nrow = ceiling(length(var_cat_levels) / 4), byrow = T)) +
...
generate_venn_diagram <- function(dt_for_venn, plot_filename) {
 ...
 for(i in all_trans_pairs) {
  ...
  el_set_row_N_for_venn <- list(dt_for_venn[get(first_element) >= VD_MIN_OVL_PROP, row_N], dt_for_venn[get(second_element) >= VD_MIN_OVL_PROP, row_N])
...
compare_element_sets <- function(element_set_cat) {
 ...
 group_id_vec <- unique(element_sets[, group_id])
 rm(element_sets)
 ...
 if(TR_NAME_LEN_MAX <= 7) {
  cols_to_modify <- colnames(element_set_groups) %chin% c("transcriptome_paste", "length_paste", "length_mean", "length_prop") == F
 ...
 write.table(comp_similarity_dt, file = paste(sub(" ", "_", element_set_cat, fixed = T), "pairwise_completeness_similarity_matrix.tsv", sep = "_"), sep = "\t", row.names =  F, col.names = F, quote = F)
 ...
 ggsave(element_set_tp, filename = paste(paste(sub(" ", "_", element_set_cat, fixed = T), "pairwise_comp_similarity_tileplot", sep = "_"), FIGURE_EXT, sep = "."), height = TP_HEIGHT, width = TP_WIDTH, dpi = FIGURE_DPI, bg = "white") 
...
compare_element_set_annot <- function(element_set_cat)  {
 ...
 write.table(cbind(gtf_set_ids, gtf_set_ovl_prop_dt), file = paste(sub(" ", "_", element_set_cat, fixed = T), "annotation_based_comp_matrix.tsv", sep = "_"), sep = "\t", row.names = F, col.names = T, quote = F)  
 rm(gtf_set_ids)
...
This HTML document contains the output of CATS-rb transcriptome assembly comparison tool. For more details on each table and figure, refer to the tool's [documentation](https://github.com/bodulic/CATS-rb).
<br>
<br>

```{r echo=F, message=F, warning=F}
#Importing exon mappings from files
exon_mappings <- import_data("mapped.tsv")
...
invisible(gc(full = T))
tr_lengths <- import_data("lengths.tsv")
...
if (transcripts[map_N != 1, .N] != 0) {
 multimapped_tr <- transcripts[tr_name %chin% unique(transcripts[map_N > 1, tr_name])]
 ...
 multimapped_tr_gr_nearest_k <- tryCatch({nearestKNeighbors(multimapped_tr_gr, k = max(multimapped_tr[, map_N]) - 1, ignore.strand = T)
}, error = function(e) { IntegerList(as.list(nearest(multimapped_tr_gr, ignore.strand = T)))})
...
write.table(str_inc_gen_pos_tr, file = "transcripts_disjunct_genomic_regions.tsv", sep = "\t", row.names = F, col.names = T, quote = F)
...
exon_N_stats <- analyse_categorical_variable(per_tr_exon_N, "exon_N", EXON_PER_TR_N_BREAKS, "Number of exons per transcript", "Transcript")
...
transcriptome_map_stat_params <- data.table(c("N, % unmapped transcripts", "Transcript alignment proportion (mean, IQR)", "N, % multimapped transcripts", "N, % structurally inconsistent transcripts", "N exons", "N exons per transcript (mean, IQR)", "Exon length (bp) (mean, IQR)"))
...
#Defining exon sets. Performing exon set genomic distribution analysis
if (STRANDED == F) {
...
EL_SET_GTF_SET_OVL_PROP_BREAKS <- as.numeric(unlist(strsplit(ext_args[, args][38], ",")))
```
...
#Writing annotation-based exon set coordinates to file
gtf_exon_set_dt <- as.data.table(gtf_exon_set_gr)[, .(gtf_set_id, seqnames, start, end)]
setnames(gtf_exon_set_dt, c("set_id", "scaff", "scaff_start", "scaff_end"))
write.table(gtf_exon_set_dt, file = "annotation_based_exon_set_coordinates.tsv", sep = "\t", row.names = F, col.names = T, quote = F)
rm(gtf_exon_set_dt)
...
#Writing annotation-based transcript set coordinates to file
gtf_transcript_set_dt <- as.data.table(gtf_transcript_set_gr)[, .(gtf_set_id, seqnames, start, end)]
setnames(gtf_transcript_set_dt, c("set_id", "scaff", "scaff_start", "scaff_end"))
write.table(gtf_transcript_set_dt, file = "annotation_based_transcript_set_coordinates.tsv", sep = "\t", row.names = F, col.names = T, quote = F)
rm(gtf_transcript_set_dt)
